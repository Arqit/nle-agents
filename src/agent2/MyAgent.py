...
class Noisy_Layer(nn.Module):
	...
	def __init__(self,input_size: int, output_size: int,init_std: float = 0.5):
		...
		#A normal linear layer would just have one weight and bias vector
		#The NoisyNet however needs one for each parameter of the gaussian it samples from
		#Hence the weight_mu and weight_sigma. 
	...
	def forward(self, x):
		...
		"""
		return F.linear( #This works in a similar fasion to a normal linear layer just with the noise factored in
			x,
	...
	def reset_noise(self):
		"""
		Make new noise. This ensures that the agent does not have a one-and-done exploration policy
		"""
